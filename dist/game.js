    ...
    for (let i = 0; i < 8; i++) {
        while (true) {
            ...
            if (!placedActors.has(coordsToKey(goblinCoords))) {
                placedActors.set(coordsToKey(goblinCoords), Object.assign({ glyph: "g", name: "Goblin" }, goblinCoords));
...
function movePlayer(game) {
    ...
    const prevPlayerCoords = Object.assign({}, game.player);
    if (nextInput) {
        if (nextInput === types_1.InputKey.UP) {
            game.player.y--;
        }
        else if (nextInput === types_1.InputKey.DOWN) {
            game.player.y++;
        }
        else if (nextInput === types_1.InputKey.LEFT) {
            game.player.x--;
        }
        else if (nextInput === types_1.InputKey.RIGHT) {
            game.player.x++;
        }
